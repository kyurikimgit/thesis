...
do
    ...
    do
		...
		do
			echo $size $instance $iter
				rm UB.csv
				rm outputNes.csv
				rm output.csv
				rm items.csv
##### Data loader
				LOADER=../CT01Loader.so    	# ../C0Loader.so ../C6Loader.so ../CXLoader.so ../SWFLoader.so ../GWFLoader.so
...
## required forSWFLoader and GWFLoader
					...
					ALGO=(../ArcFlow.so ../LSpatterns.so ../Pani.so ../Hybrid.so ../LSD_I1.so ../LSD    _I2.so ../Kou.so ../KouSort.so)
...
### Parameters for online algorithms
			...
			do
				echo loader $LOADER > config0.txt
					...
					RAND_SEED=$SEED taskset -c 5 ../VSSimKernel config0.txt
