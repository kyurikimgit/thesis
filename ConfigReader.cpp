...
ConfigReader::ConfigReader(const char* _path) {
    ...
    env.runs = 10;
    env.startJobs = 1;
    // env.file = "/home/kyurikim/vectorbinpacking/Datasets/CT01/CL_1_25_1.vbp";
    env.cpus = 1;
    env.ram = 1;
	env.maxLocalDisk = -1;
	env.maxNetwork = -1;
...
void ConfigReader::readConfigFile(const char* _path) {
    ...
    while(std::getline(file, line)) {
        ...
        if(word1 == "loader") {
            ...
            env.vbpDimension = jmd.dimensions;
        ...
        }else if(word1 == "vbpClass") {
        env.vbpClass = atoi(word2.c_str());
        } else if(word1 == "vbpSize") {
         env.vbpSize = atoi(word2.c_str());
        } else if(word1 == "vbpInstance") {
        env.vbpInstance = atoi(word2.c_str());  
        } else {
