...
//     printf("Calling loadJobs ...\n");
//     std::vector<Job>* jv = jl(env->file.c_str(), jmd, env, env->runs);
//     printf("    Jobs loaded? ... ");
//     if(!jv) {
//         printf("No jobs loaded! ...\n");
//     } else {
...
//     std::vector<double> runtimeVec;
//     std::vector<double> successVec;
//     std::vector< std::vector<int> > usedMachinesPerExperiment;
...
//     std::stringstream ss;
//     ss << algoName;
//     ss << "-Jobs:" << env->startJobs << "_" << jmd->numberOfJobs << "_" 
//        << env->stepWidth << "_" << env->runs;
//     ss << "-Mach:" << env->machines << "-Dims:" << jmd->dimensions;

//     std::vector<std::vector<std::string>> machineLoad1;
//     std::vector<std::vector<std::string>> machineLoad2;

//     Timer timer;

//     // --- 실험을 반복적으로 실행하는 외부 루프를 제거하고, job 개수를 고정하여 한 번만 실행 ---
//     {
//         // 고정된 job 개수 (예: 25)
//         int fixedJobCount = jmd->numberOfJobs;
//         printf("Experiment with %d jobs and %d machines ...\n", fixedJobCount, env->machines);
        
//         // MachineGenerator와 machineQueue는 새로 선언
//         MachineGenerator machineGen(jmd->dimensions);
//         std::vector<Machine>* machineQueue = machineGen.generateMachines(env->machines, capacity);
...
//         avgUsedMachine = 0.0;
//         double averageTime = 0.0;
//         std::vector<int> usedMachinesThisExperiment;
        
//         int j;
//         for(j = 0; j < env->runs && active; j++) {
//             // machineQueue 재설정
//             for(unsigned m = 0; m < machineQueue->size(); m++) {
//                 machineQueue->at(m).reset(capacity);
//             }
            
//             // 고정된 job 개수만큼 job 선택
//             std::vector<Job>* jq = new std::vector<Job>(jv->begin(), jv->begin() + fixedJobCount);
//             std::vector<Machine>* outMachines = nullptr;
...
//             averageTime += timer.getElapsedTimeInMicroSec();
//             int used = outMachines->size();
//             avgUsedMachine += used;
//             usedMachinesThisExperiment.push_back(used);
            
//             delete jq;
//             delete outMachines;
//         }
        
//         runtimeVec.push_back(averageTime / env->runs);
//         successVec.push_back((((double) env->runs - fails)) / ((double) env->runs) * 100.0);
//         usedMachinesPerExperiment.push_back(usedMachinesThisExperiment);
        
//         avgUsedMachine /= env->runs;
//         printf("    Final Result for %d jobs and %d machines (avg used machines: %3.2f): %3.2f%% success ...\n",
//                fixedJobCount, env->machines, avgUsedMachine, (100.0f - ((float) fails) * 100.0f / ((float) env->runs)));
        
//         printf("    Used machines per run: ");
//         for (int cnt : usedMachinesThisExperiment) {
//             printf("%d ", cnt);
//         }
//         printf("\n");
...
//     theSeed = seed;
// #endif
//     sss << "SuccessesTab-" << env->startJobs << "-" << jmd->numberOfJobs << "-" 
//         << env->stepWidth << "-S" << theSeed << ".csv";
//     tss << "TimingTab-" << env->startJobs << "-" << jmd->numberOfJobs << "-" 
//         << env->stepWidth << "-S" << theSeed << ".csv";
//     mss1 << "Machineload1-" << env->startJobs << "-" << jmd->numberOfJobs << "-" 
//          << env->stepWidth << "-S" << theSeed << ".csv";
//     mss2 << "Machineload2-" << env->startJobs << "-" << jmd->numberOfJobs << "-" 
//          << env->stepWidth << "-S" << theSeed << ".csv";
//     // CSVWriter cw = CSVWriter(ss.str(), successVec, runtimeVec, env->startJobs, 
//     //   jmd->numberOfJobs, env->stepWidth, sss.str(), tss.str(), true);
// #endif
//     return 0;
// }
...
int main(int argc, char** argv) {
    ...
    // --- 실험을 반복적으로 실행하는 외부 루프를 제거하고, job 개수를 고정하여 한 번만 실행 ---
    {
        ...
        double totalSec = averageTime / 1e6; // averageTime은 모든 run의 수행 시간 합(마이크로초 단위)
        ...
        printf("    Total runtime for all runs: %.2f seconds\n", totalSec);
