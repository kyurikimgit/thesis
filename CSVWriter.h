...
//		plt::legend();
	...
	void writeColumn(rapidcsv::Document *_doc, std::string _name, std::vector <T> &_data, unsigned _size) {
		...
		// set the column with the same algoName
		rowSize = i;
		_doc->SetCell<std::string>(rowSize, 0, _name);
		_size = _doc->GetColumn<std::string>(rowSize).size();		
	//	_doc->SetCell<T>(rowSize, _size+1, _data[0]);
		if(rowSize == 1){
			 std::vector <std::string> row = _doc->GetRow<std::string>(1);
			if(row[rowSize].empty()){
				_doc->SetCell<T>(rowSize, 1, _data[0]);
			}else{
			_doc->SetCell<T>(rowSize, _size, _data[0]);}	
		}
		else{
		for(int i = 0; i < _size; i++) {
			 std::vector <std::string> row = _doc->GetRow<std::string>(i);
			if(row[rowSize].empty()){
				_doc->SetCell<T>(rowSize, i, _data[0]);
				break;
		}}}
	}
